...
'rma_VALIDFROM_DESC' => ' {$REMOTE_MANAGEMENT} 在下面输入外部网络的主机IP，将允许这些主机远程访问server manager。使用子网掩码255.255.255.255以允许指定主机的访问。任何指定范围内的主机均通过使用HTTPS来访问server manager。 ',
...
'rma_DESC_SERIAL_CONSOLE_ACCESS' => ' 串口控制台 您可以允许一个终端通过串口连接来访问系统控制台。',
'rma_PORT1_ENABLED' => '在串口1启用',
'rma_PORT2_ENABLED' => '串口2启用',
'rma_LABEL_PPTP_ACCESS' => 'PPTP默认用户访问',
'rma_DESC_TELNET_ACCESS' => ' Telnet设置 警告: Telnet当前被启用，但此特性已不被支持了。Telnet本来就是不安全的，仅仅在没有可替代方案的情况下使用。您应该设置为 {$NO_ACCESS} ，如果远程需要访问，请使用SSH。一旦禁用，telnet将不会在屏幕上出现。 ',
